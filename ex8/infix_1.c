...
int main(int argc, const char * argv[]) {
  if (argc > 1)
  {
    char s[strlen(argv[1])]; // create char table[length of firs argv argument after ./main]
    strcpy(s, argv[1]); // copy argv[1] to s
    if(!check(s)) // if string is an acceptable sequence
    {
      printf(" error not good expression !\n");
      return 1;
    }
    char *d = conversion(s);
    printf(" infix expression: %s\n",d);
    long l = evaluation(s);
    printf(" expression evaluation: %lu\n", l);
  }
    return 0;
}
