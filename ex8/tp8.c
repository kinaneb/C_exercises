...
int main(int argc, const char * argv[]) {
  char s3[] = "(-(*50 999999333344 ) (+ 20 7))";
  char s2[150] = "(/ ( *            (* 2      (+(- 3 2 ) (+ 1 1 )) ) 5 ) 15 )";
  char s1[] = "(-(*50 3) (+ 999999999999920 7))";
  char s[150] = "(- 15 5 )";
  char *t[]= {s, s1, s2, s3};
  for(int i = 0; i < 4; i++)
  {
    ...
    printf("test: %d, le valeur de check c'est: %d\n", i+1, res);
    char *d = conversion(t[i]);
    printf("test: %d, conversion: %s\n",i+1 , d);
    long l;
    l = evaluation(t[i]);
    printf("test: %d, evaluation= %lu\n    ********\n", i+1, l);
  }
  return 0;
}
